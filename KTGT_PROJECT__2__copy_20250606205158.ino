...
const int buttonPins[] = {5, 19, 18, 5};
...
void sendDatatoMQTT() {
    StaticJsonDocument<100> gasJson;
    StaticJsonDocument<100> fireJson;

    gasJson["gas_value"] = readMQ2();
    fireJson["fire_status"] = readFireSensor();

    // Chuyển JSON thành chuỗi
    char gasBuffer[100];
    char fireBuffer[100];
    serializeJson(gasJson, gasBuffer);
    serializeJson(fireJson, fireBuffer);
  ...
  Serial.println(message);
   ...
   if(doc.containsKey("status")) {
    int status = doc["status"];
        Serial.print("Trạng thái nhận được: ");
        Serial.println(status);

        // Xử lý theo từng topic
        if (strcmp(topic, "Devices/Fan") == 0) {
            digitalWrite(RELAY1, status ? HIGH : LOW);
            controlWindow(status);
            Serial.println(status ? "Quạt BẬT" : "Quạt TẮT");
        } 
        else if (strcmp(topic, "Devices/Pump") == 0) {
            digitalWrite(RELAY2, status ? HIGH : LOW);
            Serial.println(status ? "Máy bơm BẬT" : "Máy bơm TẮT");
        }
   }
}
...
void TaskButton(void *pvParameters) {
    ...
    while (1) {
        bool currentStateBTN1 = digitalRead(5);
        bool currentStateBTN2 = digitalRead(19);
        bool currentStateBTN3 = digitalRead(18);
